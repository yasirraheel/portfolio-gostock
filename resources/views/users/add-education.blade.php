...
@section('title') {{ __('misc.add_education') }} - @endsection
			...
			<div class="col-md-9">
				...
				<div class="card shadow-custom border-0">
					<div class="card-body p-lg-4">
						<div class="row">
							<div class="col-lg-6">
								<h4 class="mb-0">{{ __('misc.add_education') }}</h4>
								<small class="text-muted">{{ __('misc.add_your_educational_background') }}</small>
							</div>
					...
					<div class="card-body p-lg-4">
						<form method="POST" action="{{ route('user.education.store') }}" enctype="multipart/form-data">
								...
								<div class="col-md-6">
									<div class="form-floating mb-3">
										<input type="text" name="institution_name" class="form-control @error('institution_name') is-invalid @enderror"
											   value="{{ old('institution_name') }}" required id="institution_name" placeholder="University of Example">
								...
								<div class="col-md-6">
									<div class="form-floating mb-3">
										<select name="education_level" class="form-select @error('education_level') is-invalid @enderror" required id="education_level">
											...
											<option value="high_school" {{ old('education_level') == 'high_school' ? 'selected' : '' }}>{{ __('misc.high_school') }}</option>
											<option value="intermediate" {{ old('education_level') == 'intermediate' ? 'selected' : '' }}>{{ __('misc.intermediate') }}</option>
											<option value="associate" {{ old('education_level') == 'associate' ? 'selected' : '' }}>{{ __('misc.associate_degree') }}</option>
											<option value="bachelor" {{ old('education_level') == 'bachelor' ? 'selected' : '' }}>{{ __('misc.bachelor_degree') }}</option>
											<option value="master" {{ old('education_level') == 'master' ? 'selected' : '' }}>{{ __('misc.master_degree') }}</option>
											<option value="doctorate" {{ old('education_level') == 'doctorate' ? 'selected' : '' }}>{{ __('misc.doctorate') }}</option>
											<option value="certificate" {{ old('education_level') == 'certificate' ? 'selected' : '' }}>{{ __('misc.certificate') }}</option>
											<option value="diploma" {{ old('education_level') == 'diploma' ? 'selected' : '' }}>{{ __('misc.diploma') }}</option>
											<option value="other" {{ old('education_level') == 'other' ? 'selected' : '' }}>{{ __('misc.other') }}</option>
										</select>
								...
								<div class="col-md-6">
									<div class="form-floating mb-3">
										<input type="text" name="degree" class="form-control @error('degree') is-invalid @enderror"
											   value="{{ old('degree') }}" required id="degree" placeholder="Bachelor of Science">
								...
								<div class="col-md-6">
									<div class="form-floating mb-3">
										<input type="text" name="field_of_study" class="form-control @error('field_of_study') is-invalid @enderror"
											   value="{{ old('field_of_study') }}" required id="field_of_study" placeholder="Computer Science">
								...
								<div class="col-md-6">
									<div class="form-floating mb-3">
										<input type="date" name="start_date" class="form-control @error('start_date') is-invalid @enderror"
											   value="{{ old('start_date') }}" required id="start_date">
							...
							<div class="form-check form-switch form-switch-md mb-3">
								...
								<small class="text-muted d-block">{{ __('misc.leave_blank_if_current') }}</small>
							</div>
								...
								<div class="col-md-6">
									<div class="form-floating mb-3">
										<input type="text" name="location" class="form-control @error('location') is-invalid @enderror"
											   value="{{ old('location') }}" id="location" placeholder="City, Country">
								...
								<div class="col-md-6">
									<div class="form-floating mb-3">
										<input type="text" name="grade" class="form-control @error('grade') is-invalid @enderror"
											   value="{{ old('grade') }}" id="grade" placeholder="3.8 GPA, First Class Honours">
							...
							<div class="form-floating mb-3">
								<input type="url" name="website" class="form-control @error('website') is-invalid @enderror"
									   value="{{ old('website') }}" id="website" placeholder="https://university.edu">
							...
							<div class="form-floating mb-3">
								<textarea name="description" class="form-control @error('description') is-invalid @enderror"
										  style="height: 120px" id="description" placeholder="{{ __('misc.describe_your_education') }}">{{ old('description') }}</textarea>
							...
							<div class="form-floating mb-3">
								<textarea name="activities" class="form-control @error('activities') is-invalid @enderror"
										  style="height: 80px" id="activities" placeholder="{{ __('misc.extracurricular_activities') }}">{{ old('activities') }}</textarea>
								<label for="activities">{{ __('misc.activities') }}</label>
								<small class="text-muted d-block">{{ __('misc.separate_with_commas') }}</small>
								@error('activities')
									<div class="invalid-feedback">{{ $message }}</div>
								@enderror
							</div>
							...
							<div class="mb-4">
								...
								<small class="d-block text-muted">
									<i class="bi bi-info-circle me-1"></i>{{ __('misc.max_file_size_1mb') }}
								</small>
								@error('logo')
									<div class="invalid-feedback">{{ $message }}</div>
								@enderror
							...
							<div class="d-flex gap-2">
								<button type="submit" class="btn btn-custom">
									<i class="bi bi-check2 me-1"></i>{{ __('misc.save_education') }}
								</button>
